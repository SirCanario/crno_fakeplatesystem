...
RegisterNetEvent('crno_fakeplatesystem:changePlate', function(canRemove)
    ...
    if not IsPedInAnyVehicle(PlayerPedId()) then
        ...
        if vehicle ~= 0 and distance < 3.5 then
            ...
            if canRemove then
                ...
                TriggerServerEvent('crno_fakeplatesystem:removeItem')
    ...
    if not foundInBlackList then
        ...
        if vehicle ~= 0 and distance < 3.5 then
            ...
            if plateCleanned == data.plate then
                ...
                exports.crno_progressbar:Progress(_U('changing_plate'), Config.Time)

                Wait(Config.Time)

                ClearPedTasks(PlayerPedId())

                ESX.TriggerServerCallback('crno_fakeplatesystem:updatePlate', function() end, plateCleanned, data.plate)
...
RegisterNetEvent('crno_fakeplatesystem:updatePlate', function(coords, oldPlate, newPlate)
